...
const Deployment = () => {
    ...
    useEffect(() => {
        const fetchLogs = async () => {
            try {
                const depLogs = await api.deployments.getLogs(
                    Number(deploymentId),
                );
                const normalizedLogs = depLogs.map((log: unknown) =>
                    typeof log === "string"
                        ? log
                        : typeof (log as { message?: string })?.message ===
                            "string"
        ...
        const fetchStatus = async () => {
            try {
                const depStatus = await api.deployments.getStatus(
                    Number(deploymentId),
                );
                    ...
                    >
                        ...
                        <div className="text-emerald-500 mb-3">
                            ...
                            <span className="text-neutral-400">
                                gitdrop deploy --id {deploymentId}
                            </span>
                        </div>
